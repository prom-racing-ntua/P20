	...
	def update(self,*args):
		if self.by_meters:
			for i in range(self.num_of_plots):
				...
				self.labs[i].text = self.ylabs[i]%(self.points_list_m[i][-1][1]*self.scale[i]+self.offset[i])
				self.labs[i].color[3]=1
				self.labs[i].bgclr[3]=1
		else:
			for i in range(self.num_of_plots):
				self.diagrams[i].points = self.points_list_t[i]
				self.labs[i].text = self.ylabs[i]%(self.points_list_t[i][-1][1]*self.scale[i]+self.offset[i])
				self.labs[i].color[3]=1
				self.labs[i].bgclr[3]=1
